...
import type { LifecycleTimelineStep } from "@/lib/lifecycle/lifecycleTimeline";
...
export function LifecycleTimeline({
  ...
  if (steps.length === 0) {
    return <p className="text-xs text-muted">Select a mission to view its lifecycle timeline.</p>;
  }

  return (
    <ol className={compact ? "space-y-2" : "space-y-3"}>
      {steps.map((step, index) => (
        <li key={step.stage} className="relative pl-6">
